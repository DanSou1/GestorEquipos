...
- Several lookup tables (`Ram`, `OSVersion`, `MaintenanceType`, `Area`, `Regional`) have no admin UI by design (out of scope) — seed them directly via SQL when setting up a fresh dev database, or the `Desktop`/`Maintenance` create forms will have empty dropdowns. `Remote` is the one exception: it still has no dedicated CRUD page, but `Desktop/Create` and `Edit` can create a new row inline (see Architecture below) — you only need to seed it via SQL if you want pre-existing rows available to reuse from the dropdown.
...
- Core entities (`Models/*.cs`): `Users` (a person), `UserSystem` (a login account: `Username`/`PasswordHash`/`Rol`, one-to-one with `Users` via a unique index on `UserId` despite the model nominally supporting a collection), `Rol` (`Administrador` or `Auditor`), `Area`, `Regional`, `Desktop` (a physical computer — `OSVersion`, `Ram`, optional `Remote`, `Estado` bool for Activo/Inactivo), `Remote` (a desktop's remote-access info — `ConnectionType` enum `Aplicativo`/`EscritorioRemotoWindows`; `Aplicativo` only uses `AppDescription`, `EscritorioRemotoWindows` requires `IPAddress`/`Port`/`Username`/`Password` — enforced by a DB `CHECK` constraint added `WITH NOCHECK` so it doesn't retroactively validate rows seeded before this constraint existed; `Password` is stored in plaintext by explicit product decision, there is no encryption anywhere in this app, and the whole "Acceso remoto" section is gated to `Administrador` in `Details`/PDF unlike most other read-only content), `Asignation` (append-only join: one row per assignment event — the "current" assignment for a desktop is simply its most recent `Asignation` by date, there's no separate `IsCurrent` flag), `Maintenance`/`MaintenanceType` (service history per `Desktop`, `Maintenance.TechnicianUserSystemId` is a required FK to `UserSystem`), `Peripheral`/`PeripheralType` (peripherals belong to a `Desktop`, with their own `Estado` enum — `Activo`/`Inactivo`/`Raes` — **independent** of the desktop's Estado), `PeripheralObservation` (append-only event log per peripheral: `Reparacion`/`Cambio`/`BajaRAES`, drives `Peripheral.Estado` transitions), `License` (per-`Desktop`, free-text `SoftwareType` + optional `LicenseKey` + `NoLicense` flag for legitimately-unlicensed software), `SpecChangeLog` (append-only audit trail of `Desktop` spec edits: field name, old/new value, who, when).
...
- `IDesktopService`/`DesktopService`: `GetAllAsync` (equipment list, latest-assignment-per-desktop via a correlated subquery, not N+1 loops), `GetDetailAsync` (full hoja de vida), `CreateAsync`, `UpdateSpecsAsync` (diffs every editable field against the current row and writes a `SpecChangeLog` entry per change before applying it), `DeactivateAsync` (RAES transition, delegates the actual reassignment to `IAsignationService` rather than duplicating that logic). `CreateAsync`/`UpdateSpecsAsync` also resolve `DesktopCreateViewModel.RemoteSelection` (`""` / an existing `Remote.Id` / `"new"`) via the private `ResolveRemoteSelection` helper — when `"new"`, a `Remote` is built from the `NewRemote*` fields and assigned through the `Desktop.Remote` navigation property (not `RemoteId` directly) so EF Core inserts it and wires the FK in the same `SaveChangesAsync()`; there is deliberately no way to edit an already-created `Remote`'s own fields in place, only to switch a `Desktop` to reuse an existing one or create a new one.
